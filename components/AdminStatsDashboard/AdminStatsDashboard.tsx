...
import { getAdminStats, type AdminStats, type AdminRaffleStats } from '@/lib/api/stats';
...
export default function AdminStatsDashboard() {
  ...
  const parsePrizeString = (prizeString: string): { name: string; image?: string } => {
    ...
    <div className="space-y-6">
      ...
      <div className="bg-gradient-to-r from-blue-600 to-blue-700 dark:from-blue-700 dark:to-blue-800 rounded-2xl shadow-xl p-8 text-white relative">
        ...
        <p className="text-blue-100 text-lg mb-6">
          ...
          <a
            href={`https://${stats.client.domain}.rifasuerte.com`}
            target="_blank"
            rel="noopener noreferrer"
            className="text-white hover:text-blue-100 hover:underline transition-colors inline-flex items-center gap-1"
          >
            {stats.client.domain}.rifasuerte.com
        ...
        <div className="flex items-center gap-6">
          <div className="bg-white/20 backdrop-blur-sm rounded-xl px-6 py-4">
            <div className="text-blue-100 text-sm font-medium mb-1">Total de Rifas</div>
            <div className="text-4xl font-bold">{stats.totalRaffles}</div>
          </div>
          <div className="bg-white/20 backdrop-blur-sm rounded-xl px-6 py-4">
            <div className="text-blue-100 text-sm font-medium mb-1">Rifas Activas</div>
            <div className="text-4xl font-bold text-green-200">{stats.activeRaffles}</div>
          </div>
          <div className="bg-white/20 backdrop-blur-sm rounded-xl px-6 py-4">
            <div className="text-blue-100 text-sm font-medium mb-1">Rifas Cerradas</div>
            <div className="text-4xl font-bold">{stats.closedRaffles}</div>
          </div>
        </div>
      </div>
      ...
      <div className="space-y-6">
        {stats.raffles.map((raffle) => {
          ...
          return (
            ...
            >
              {/* Header de la rifa */}
              <div className={`bg-gradient-to-r from-gray-50 to-gray-100 dark:from-gray-700 dark:to-gray-800 px-6 py-5 ${isMinimized ? '' : 'border-b border-gray-200 dark:border-gray-700'}`}>
                {/* Card de innerCode si existe */}
                {raffle.innerCode && (
                  <a
                    href={`https://${stats.client.domain}.rifasuerte.com/rifa/${raffle.innerCode}`}
                    target="_blank"
                    rel="noopener noreferrer"
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <button
                        onClick={() => {
                          ...
                          setSelectedRaffle({
                            innerCode: raffle.innerCode,
                            name: raffle.name,
                            domain: stats.client.domain,
                          });
              ...
              <div className="p-6 space-y-6">
                ...
                <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                  ...
                  <div className="bg-gray-50 dark:bg-gray-900/50 rounded-xl p-4">
                    ...
                    <div className="text-2xl font-bold text-gray-900 dark:text-white">
                      {formatCurrency(parseFloat(raffle.ticketPrice), raffle.ticketCurrency)}
                    </div>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 pt-4 border-t border-gray-200 dark:border-gray-700">
                  <div>
                    ...
                    <div className="space-y-2 text-sm">
                      <div className="flex justify-between">
                        <span className="text-gray-600 dark:text-gray-400">Fecha del Sorteo:</span>
                        <span className="font-medium text-gray-900 dark:text-white">
                          {formatDate(raffle.date)}
                        </span>
                      </div>
                      <div className="flex justify-between">
                        <span className="text-gray-600 dark:text-gray-400">Número de Ganadores:</span>
                        <span className="font-medium text-gray-900 dark:text-white">
                  ...
                  <div>
                    ...
                    <div className="space-y-2">
                      {raffle.prizes.map((prizeString, index) => {
                        const prize = parsePrizeString(prizeString);
                        return (
                          <div
                            key={index}
